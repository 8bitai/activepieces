...
const jsonParser = (_req: FastifyRequest, body: string, done: (err: Error | null, body?: unknown) => void): void => {
...
export const webhookModule: FastifyPluginAsync = async (app) => {
    // Match exact 'application/json' and duplicate/combined values (e.g. 'application/json, application/json')
    // to avoid 415 when clients send multiple Content-Type headers (e.g. from saved curl replay).
